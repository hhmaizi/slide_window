...
{
    // no marginalization for now, do it later.
...
//     Headers[frame_count] = header;
	...
	// cam imu extrinsic: 0: fix extrinsic param, 
	// 1: optimize around initial guess, 2: no prior, calibrate extrinsic.
    ...
    {// since done with initialize, let's solveOdometry.
        TicToc t_solve;
        solveOdometry();
...
// feature[i].feature_per_frame saves all pts_i(feature i)'s match in other frames(images).
    ...
    for (auto &it_per_id : f_manager.feature)
    {
        it_per_id.used_num = it_per_id.feature_per_frame.size();// seen by how many frames?
        if (!(it_per_id.used_num >= 2 && it_per_id.start_frame < WINDOW_SIZE - 2))
            continue;
    ...
    relocalize = false;
    //loop close factor, removed by ted for now. Maybe, add this later.
...
//     ROS_DEBUG("solver costs: %f", t_solver.toc());
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
        ...
        // add imu_factor for marginalization.
        {
        ...
        // add ProjectionFactor for marginalization.
        {
            int feature_index = -1;
            for (auto &it_per_id : f_manager.feature)
            {
    ...
    else 
    {
        ...
        {// para_Pose[WINDOW_SIZE - 1]: the last keyframe in sliding window?
			// para_Pose[WINDOW_SIZE]: the current frame?
