...
// 		sqrt_info.setIdentity();
        ...
        if (jacobians)
        {
			...
			// preparements for computing jacobian
			Eigen::Matrix3d I3x3 = Matrix3d::Identity();   // I_3x3
			Eigen::Matrix3d O3x3 = Matrix3d::Zero();       // 0_3x3
			...
			Eigen::Vector3d rPhiij = 2 * (corrected_delta_q.inverse() * (Qi.inverse() * Qj)).vec();
// 			Eigen::Vector3d rPhiij = _error.segment<3>(6); // residual of rotation, rPhiij
			Eigen::Matrix3d JrInv_rPhi = Sophus::SO3::JacobianRInv(rPhiij);    // inverse right jacobian of so3 term #rPhiij#
// 			Eigen::Matrix3d J_rPhi_dbg = M.getJRBiasg();
			Matrix3d ExprPhiijTrans = Sophus::SO3::exp(rPhiij).inverse().matrix();
			Matrix3d JrBiasGCorr = Sophus::SO3::JacobianR(dq_dbg * (Bgi - linearized_bg));

			// Eigen::Matrix3d JrInv_rPhi = Sophus::SO3::JacobianRInv(residual.segment<3>());
            if (jacobians[0])
            {
                Eigen::Map<Eigen::Matrix<double, 15, 7, Eigen::RowMajor>> jacobian_pose_i(jacobians[0]);
                jacobian_pose_i.setZero();

//                 jacobian_pose_i.block<3, 3>(O_P, O_P) = -Qi.inverse().toRotationMatrix();// why not I3x1?
                jacobian_pose_i.block<3, 3>(O_P, O_P) = -I3x3;// why not I3x1?
                jacobian_pose_i.block<3, 3>(O_P, O_R) = Utility::skewSymmetric(Qi.inverse() * (Pj - Pi - Vi * sum_dt + 0.5 * G * sum_dt * sum_dt));

#if 0
            jacobian_pose_i.block<3, 3>(O_R, O_R) = -(Qj.inverse() * Qi).toRotationMatrix();
#else
				jacobian_pose_i.block<3, 3>(O_R, O_R) = - JrInv_rPhi * RjT * Ri; ;  //dphi_i
				// Qleft Qright 四元数的左乘与右乘， 参见quaternion kinematics
//                 jacobian_pose_i.block<3, 3>(O_R, O_R) = -(Utility::Qleft(Qj.inverse() * Qi) * Utility::Qright(corrected_delta_q)).bottomRightCorner<3, 3>();
#endif
				jacobian_pose_i.block<3, 3>(O_V, O_R) = Sophus::SO3::hat( RiT * (Vj - Vi + G * sum_dt) );
//                 jacobian_pose_i.block<3, 3>(O_V, O_R) = Utility::skewSymmetric(Qi.inverse() * (G * sum_dt + Vj - Vi));
            ...
            if (jacobians[1])
            {
                ...
                jacobian_speedbias_i.block<3, 3>(O_P, O_V - O_V) = -RiT * sum_dt;
                jacobian_speedbias_i.block<3, 3>(O_P, O_BA - O_V) = -dp_dba;
                jacobian_speedbias_i.block<3, 3>(O_P, O_BG - O_V) = -dp_dbg;

#if 0
			// approx JrPhi_dbg, in the later we will give a more precite one. See manifold pre_integration for more info.
            jacobian_speedbias_i.block<3, 3>(O_R, O_BG - O_V) = -dq_dbg;
#else
//                 Eigen::Quaterniond corrected_delta_q = delta_q * Utility::deltaQ(dq_dbg * (Bgi - linearized_bg));
// 				Eigen::Quaterniond corrected_delta_q = delta_q;
				// 这部分是否可以直接借鉴vins-mono？
				   // J_rPhiij_xxx_j for Vertex_Bias_i
				jacobian_speedbias_i.block<3, 3>(O_R, O_BG - O_V) = - JrInv_rPhi * ExprPhiijTrans * JrBiasGCorr * dq_dbg; 
//                 jacobian_speedbias_i.block<3, 3>(O_R, O_BG - O_V) = -Utility::Qleft(Qj.inverse() * Qi * corrected_delta_q).bottomRightCorner<3, 3>() * dq_dbg;
#endif

                jacobian_speedbias_i.block<3, 3>(O_V, O_V - O_V) = -RiT;
            ...
            if (jacobians[2])
            {
                ...
                jacobian_pose_j.block<3, 3>(O_P, O_P) = RiT * Rj;
...
//                 jacobian_pose_j.block<3, 3>(O_R, O_R) = Utility::Qleft(corrected_delta_q.inverse() * Qi.inverse() * Qj).bottomRightCorner<3, 3>();
            ...
            if (jacobians[3])
            {
                ...
                jacobian_speedbias_j.block<3, 3>(O_V, O_V - O_V) = RiT;
